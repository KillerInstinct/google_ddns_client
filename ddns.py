...
def getlastip():
    try:
        with open(hist, "r") as old:
            lastip = old.read()
        logging.debug("Last IP Read from history file: {0}".format(lastip))
        return lastip
...
def checkipupdate(ip):    
    ...
    while status != "200":
        ...
        if getip.status_code == 200:
            ...
            url = ("https://domains.google.com/nic/update?hostname={0}"
                   "&myip={1}".format(host, newip)) 
            ddns = requests.get(url, headers=headers, auth=(uid,pwd))
            if ddns.status_code == 200:
                data = ddns.text
                cmd = data.split()
                if cmd[0] == "good":
                    logging.info("IP Updated: {0}".format(cmd[0]))
                if cmd[0] == "nochg":
                ...
                if cmd[0] == "badagent":
                    logging.error("The DDNS client is making bad requests. Ensure "
                                  ...
                                  "you're only attempting to set an IPv4 address.")
                if cmd[0] == "abuse":
                ...
                logging.error("domains.google.com returned status code: {0}".format(
                              ddns.status_code))
                return "retry"
